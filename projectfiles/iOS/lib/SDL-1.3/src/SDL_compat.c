...
SDL_VideoModeOK(int width, int height, int bpp, Uint32 flags)
{
    ...
    if (!(flags & SDL_FULLSCREEN)) {
		...
		SDL_PixelFormatEnumToMasks(mode.format, &mode_bpp, &Rmask, &Gmask, &Bmask, &Amask);
        return mode_bpp;
    }
...
SDL_SetVideoMode(int width, int height, int bpp, Uint32 flags)
{
    ...
    if (SDL_VideoWindow) {
		if (!(SDL_VideoWindow->flags & SDL_WINDOW_FULLSCREEN)) {
    ...
    window_flags |= SDL_WINDOW_BORDERLESS; //dailin
	...
	// update width and height. landscape mode may be upside.
	width = window_w; // SDL_VideoWindow->w;
	height = window_h; // SDL_VideoWindow->h;
...
void SDL_ShowKeyboard(SDL_bool show)
{
	Android_JNI_SetActivityTitle("keyboard");
}
#else
void SDL_ShowKeyboard(SDL_bool show)
{
...
SDL_SetTimer(Uint32 interval, SDL_OldTimerCallback callback)
{
	...
	if ((interval & 0xfffffff0) == 0xbbbbbbb0) {
		SDL_bool show = (interval & 0xf)? SDL_TRUE: SDL_FALSE;
		SDL_ShowKeyboard(show);
		return 0;
	}
